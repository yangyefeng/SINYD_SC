...
if %res% == 1 (
    echo [%DATE%%TIME%][重新加载配置文件][加载成功] >>../RemoteLog/script.log
) else (
    if %res% == 2 (
        echo [%DATE%%TIME%][重新加载配置文件][加载失败，采集程序未启动] >>../RemoteLog/script.log
    ) else (
        echo [%DATE%%TIME%][重新加载配置文件][加载失败] >>../RemoteLog/script.log
    )
)