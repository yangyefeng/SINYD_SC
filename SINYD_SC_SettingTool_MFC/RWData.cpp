...
void CRWData::_DeleteFile(char *filename)
{
...
bool CRWData::_CreateFile(char *filename, char *wstr)
{
	FILE *stream;
...
//清空文件内容
void CRWData::_ClearFileContent(char *filename)
{
    char temp[1024]={0};
...
//将信息追加写入日志
void CRWData::_InsertLog(char *logString,char *filePath)
{
    char cmd[1024]={0};
...
//写配置文件
bool CRWData::_WriteProfileString(char lpAppName[], char lpKeyName[], char lpString[], char lpFileName[])
{
	FILE *stream;
...
bool CRWData::_WriteProfileInt(char lpAppName[], char lpKeyName[], int value, char lpFileName[])
{
	FILE *stream;
	void *pVoid = NULL; //文件的后半部分
	bool bFindKey = false;
...
//从配置文件中获取String值
int CRWData::_GetProfileString(const char lpAppName[], const char lpKeyName[], const char lpDefault[], char lpReturnedString[], const char lpFileName[])
{
...
//从配置文件中获取Int值
int CRWData::_GetProfileInt(const char lpAppName[], const char lpKeyName[], int nDefault, const char lpFileName[])
{
    int ret = nDefault;
    FILE *stream;
    bool bFindVal = false;
...
//移除空格
char* CRWData::remove_empty (char *p)
{
    char * ptmp = p;
    while (*ptmp == ' ')
    {
...
void CRWData::_WriteDebugForLog(char logstr[], char FileName[])
{
    FILE *stream;
    stream = fopen(FileName, "a");
    if ( NULL == stream ) {
