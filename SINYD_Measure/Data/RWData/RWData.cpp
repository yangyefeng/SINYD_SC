...
void CRWData::DeleteFile(char *filename)
{
...
bool CRWData::CreateFile(const char *filename, const char *wstr)
{
	FILE *stream;
...
//清空文件内容
void CRWData::ClearFileContent(char *filename)
{
    char temp[1024]={0};
...
//将信息追加写入日志
void CRWData::InsertLog(char *logString,char *filePath)
{
    char cmd[1024]={0};
...
//将信息追加写入日志
void CRWData::RepeatLog(char *logString,char *filePath)
{
    char cmd[1024]={0};
    sprintf(cmd,"echo '%s' > %s ",logString,filePath);
    system(cmd);
...
//写配置文件
bool CRWData::WriteProfileString(char lpAppName[], char lpKeyName[], char lpString[], char lpFileName[])
{
	FILE *stream;
...
//从配置文件中获取String值
INT32 CRWData::GetProfileString(const char lpAppName[], const char lpKeyName[], const char lpDefault[], char lpReturnedString[], const char lpFileName[])
{
...
//从配置文件中获取Int值
INT32 CRWData::GetProfileInt(const char lpAppName[], const char lpKeyName[], int nDefault, const char lpFileName[])
{
    INT32 ret = nDefault;
    FILE *stream;
    bool bFindVal = false;
...
void CRWData::WriteDebugForLog(INT8 logstr[], INT8 FileName[])
{
    FILE *stream;
    stream = fopen(FileName, "a");
    if ( NULL == stream ) {
