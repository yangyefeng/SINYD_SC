...
void CBitmapEx::Create(long width, long height)
{
	...
	memset(m_lpData, 255, m_dwSize * sizeof(BYTE));//默认为白色图
...
void CBitmapEx::Load(LPTSTR lpszBitmapFile)
{
	...
	if (lpszBitmapFile != NULL)
	{
		// Open .BMP file
		FILE* file = nullptr;
		errno_t err = _tfopen_s(&file, lpszBitmapFile, _T("rb"));
		if (file != NULL)
		{
			// Deinit members
			if (m_lpData != NULL)
...
void CBitmapEx::Save(LPTSTR lpszBitmapFile)
{
	...
	if (lpszBitmapFile != NULL)
	{
		...
		// Open .BMP file
		FILE* file = nullptr;
		errno_t err = _tfopen_s(&file, lpszBitmapFile, _T("wb"));
		if (err == 0 && file != NULL)
		{
...
void CBitmapEx::_RotateNearestNeighbour(long degrees, _PIXEL bgColor)
{
	...
	if (IsValid())
	{
		...
		long _width = (long)(fabs((float)m_bih.biWidth*cos(_angle)) + fabs((float)m_bih.biHeight*sin(_angle)) + 0.5f);
		long _height = (long)(fabs((float)m_bih.biWidth*sin(_angle)) + fabs((float)m_bih.biHeight*cos(_angle)) + 0.5f);
		long _pitch = m_iBpp * _width;
		while ((_pitch & 3) != 0)
			_pitch++;
...
void CBitmapEx::BlackAndWhite2(long offset)
{
	// Check for valid bitmap
	if (IsValid())
	{
		...
		for (long i = 0; i < m_bih.biHeight; i++)
		{
			...
			for (long j = 0; j < m_bih.biWidth; j++)
			{
				...
				//原来正好相反
				if (f_value > f_offset)
					lpDstData[dwTotalOffset >> 2] = _RGB(255, 255, 255);//_RGB(0,0,0);
				else
					lpDstData[dwTotalOffset >> 2] = _RGB(0, 0, 0);//_RGB(255,255,255);
...
void CBitmapEx::Binary_OTSU(int &iThreshold)//通过大津法将灰度图转化为二值图
{
	// Check for valid bitmap
	if (IsValid())
	{
// 		vector <int> vGray;//存储灰度值
// 		UCHAR *pData = (UCHAR *)m_lpData;
...
// 			{
// 				vGray.push_back(pData[i * iWidth * sizeof(DWORD) + j * sizeof(DWORD)]);
// 			}
// 		}
		UCHAR *vGray = (UCHAR*)m_lpData;
		//大津法求阈值
		...
		for (int iT = 0; iT < 255; iT++)
		{
			...
			int gray1 = 0;
			int iCount = 0;
			for (int i = 0; i < /*vGray.size()*/m_dwSize; i++)
			{
				if (vGray[i] > iT)
				{
					gray0 += vGray[i];
			...
			w0 = (float)iCount / /*vGray.size()*/m_dwSize;
			w1 = 1 - w0;
			float fVariance = w0 * w1 *
				((float)gray0 / iCount - (float)gray1 / (/*vGray.size()*/m_dwSize - iCount)) *
				((float)gray0 / iCount - (float)gray1 / (/*vGray.size()*/m_dwSize - iCount));
		...
		BlackAndWhite2(iThreshold);
	}
}

void  CBitmapEx::Binary_RBG(DWORD rgb, int iThreshold)
{
	if (IsValid())
	{
		UCHAR *pData = (UCHAR *)m_lpData;
		UCHAR *pRGB = (UCHAR *)&rgb;
		int iWidth = m_bih.biWidth;
		int iHeight = m_bih.biHeight;
		int iR = pRGB[0];
		int iG = pRGB[1];
		...
		for (int i = 0; i < iHeight; i++)
		{
			for (int j = 0; j < iWidth; j++)
			{
				int R = pData[i * iWidth * sizeof(DWORD) + j * sizeof(DWORD)];
				int G = pData[i * iWidth * sizeof(DWORD) + j * sizeof(DWORD) + 1];
				int B = pData[i * iWidth * sizeof(DWORD) + j * sizeof(DWORD) + 2];
				iDistance = sqrt((iR - R) * (iR - R) + (iG - G) * (iG - G) + (iB - B) * (iB - B));
				int iValue = 0;
				...
				pData[i * iWidth * sizeof(DWORD) + j * sizeof(DWORD)] = iValue;
				pData[i * iWidth * sizeof(DWORD) + j * sizeof(DWORD) + 1] = iValue;
				pData[i * iWidth * sizeof(DWORD) + j * sizeof(DWORD) + 2] = iValue;
			}
		}
	}
}

void  CBitmapEx::Gray_RGB(DWORD rgb)//根据指定RGB值 将彩色图转化为灰度图
{
	if (IsValid())
	{
		UCHAR *pData = (UCHAR *)m_lpData;
		...
		for (int i = 0; i < iHeight; i++)
		{
			for (int j = 0; j < iWidth; j++)
			{
				...
				int G = pData[i * iWidth * sizeof(DWORD) + j * sizeof(DWORD) + 1];
				int B = pData[i * iWidth * sizeof(DWORD) + j * sizeof(DWORD) + 2];
				iGray = sqrt((iR - R) * (iR - R) + (iG - G) * (iG - G) + (iB - B) * (iB - B)) / sqrt(3);
				pData[i * iWidth * sizeof(DWORD) + j * sizeof(DWORD)] = iGray;
				pData[i * iWidth * sizeof(DWORD) + j * sizeof(DWORD) + 1] = iGray;
				pData[i * iWidth * sizeof(DWORD) + j * sizeof(DWORD) + 2] = iGray;
			}
		}
	}
}
...
void CBitmapEx::_DrawNearestNeighbour2(long dstX, long dstY, long dstWidth, long dstHeight, CBitmapEx& bitmapEx, long srcX, long srcY, long srcWidth, long srcHeight)
{
	// Check for valid bitmap
	if (IsValid() && bitmapEx.IsValid())
	{
		...
		if (srcY < 0)
			_dstStartY = max(0, min(m_bih.biHeight - 1, dstY - srcY));
		//修改终点计算方式
		long _dstEndX = max(0, min(dstX + dstWidth - 1, m_bih.biWidth - 1));
		long _dstEndY = max(0, min(dstY + dstHeight - 1, m_bih.biHeight - 1));
		long _dstWidth = _dstEndX - _dstStartX + 1;
		long _dstHeight = _dstEndY - _dstStartY + 1;
		...
		if (dstY < 0)
			_srcStartY = max(0, min(bitmapEx.GetHeight() - 1, srcY - dstY));
		long _srcEndX = max(0, min(srcX + srcWidth - 1, bitmapEx.GetWidth() - 1));
		long _srcEndY = max(0, min(srcY + srcHeight - 1, bitmapEx.GetHeight() - 1));
		long _srcWidth = _srcEndX - _srcStartX + 1;
		long _srcHeight = _srcEndY - _srcStartY + 1;
...
void CBitmapEx::Draw2(long dstX, long dstY, long dstWidth, long dstHeight, CBitmapEx& bitmapEx, long srcX, long srcY, long srcWidth, long srcHeight)
{
	// Check for resampling mode
	switch (m_ResampleMode)
	{
	...
	{
		// Draw bitmap using nearest neighbour interpolation algorithm
		_DrawNearestNeighbour2(dstX, dstY, dstWidth, dstHeight, bitmapEx, srcX, srcY, srcWidth, srcHeight);
	}
	break;
...
void CBitmapEx::Get24BPPData(LPBYTE lpData, DWORD* pdwSize)
{
	// Check for valid bitmap
	if (IsValid())
	{
...
// 			_pitch++;
		...
		// Create temporary bitmap
		DWORD dwSize = _pitch * m_bih.biHeight;
		if (lpData != nullptr && *pdwSize >= dwSize)
		{
			// Convert bitmap
			DWORD dwDstHorizontalOffset;
			DWORD dwDstVerticalOffset = 0;
...
void CBitmapEx::GetPixels(long x, long y, long width, long height, DWORD* pdwPixelBuf, long* inoutBufLength, long* outWidth, long* outHeight)
{
	if (!pdwPixelBuf || *inoutBufLength < (width* height))
	{
		return;
	}

	memset(pdwPixelBuf, 0, *inoutBufLength);
	...
	if (IsValid())
	{
		...
		for (long i = 0; i < _height; i++)
		{
			for (long j = 0; j < _width; j++)
			{
				DWORD dwTotalOffset = (m_bih.biHeight - 1 - (i + _y1))*m_iPitch + (j + _x1)*m_iBpp;
				pdwPixelBuf[i * _width + j] = lpSrcData[dwTotalOffset >> 2];
			}
		}
		*inoutBufLength = _width*_height;
		*outWidth = _width;
		*outHeight = _height;
	}
}
