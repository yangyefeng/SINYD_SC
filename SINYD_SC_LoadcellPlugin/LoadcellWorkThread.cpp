/////////////////////////////////////////////////////////////////////////////
/// @File name: LoadcellWorkThread.cpp
/// @Author: YaoDi
...
void CTaskListWaitingForWork::GetNextTask(bool* isLastTask, shared_ptr<CLoadcellTask>& spTask)
{
	CriticalSectionLock lock(m_TaskListWaitingWorkCS);

/*	shared_ptr<CLoadcellTask> spTask(nullptr);*/
...
DWORD CLoadcellWorkThread::Execute()
{
	...
	while (true)
	{
		...
		else if (WAIT_OBJECT_0 + 1 == dw)
		{
			...
			shared_ptr<CLoadcellTask> spTask;
			m_spTaskListForWork->GetNextTask(&isLastTask, spTask);
			while (spTask != nullptr)
			{
				spTask->Execute(m_pIScreenAreaReader, m_pPluginExecuteCB, m_packageID, m_TimeStamp);
				if (isLastTask)
				{
					if (m_pPluginExecuteCB)
					{
						m_pPluginExecuteCB->DecreasePluginCount();
					}
				}
				m_spTaskListForWork->GetNextTask(&isLastTask, spTask);
			}
...
void CLoadcellWorkThread::Work(shared_ptr<CTaskListWaitingForWork> spTaskListForWork, IScreenReader* pIScreenAreaReader, IPluginExecuteCallback* pPluginExecuteCB, int packageID, const string& TimeStamp)
{
	m_spTaskListForWork = spTaskListForWork;
	m_pIScreenAreaReader = pIScreenAreaReader;
	m_pPluginExecuteCB = pPluginExecuteCB;
